...
let post_form (post_body: Tidy_email.Email.body) status body ?headers ~params uri =
  ...
  Alcotest.(check string) "The correct API endpoint is used." "https://api.com/messages" (Uri.to_string uri);
  check_headers "A basic auth header is supplied." expected_headers headers;
  Alcotest.(check (list string)) "The expected parameters are present." expected_params actual_params;
