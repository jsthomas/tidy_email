...
let expected_request_body b =
  ...
  let content_type, content =
    ...
    | Email.Mixed (_, h, _) -> ("text/html", h) in
...
(* This mock allows us to represent different status/body combinations
   ...
   function. *)
let post (content : Email.body) status (response_body : Body.t)
    ?(body : Body.t option) ?headers uri =
  ...
  let%lwt actual_post_body =
    ...
    | Some b -> Body.to_string b in
  Alcotest.(check string)
    "The correct API endpoint is used." base_url (Uri.to_string uri);
